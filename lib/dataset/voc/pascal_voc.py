...
class PascalVoc(ImageDataset):
    ...
    def image_path_at(self, id):
        image_path = os.path.join('/root/data/virtual_Data/unreal_5000_default/train',
                                  str(id) + '.png')
    ...
    def _load_annotation(self, idx, id):
        ...
        for idx, obj in enumerate(objects):
            ...
            # Start coord is 0
            x1 = max(int(bndbox.find('xmin').text) - 1,0)
            y1 = max(int(bndbox.find('ymin').text) - 1,0)
            x2 = max(int(bndbox.find('xmax').text) - 1,0)
            y2 = max(int(bndbox.find('ymax').text) - 1,0)
