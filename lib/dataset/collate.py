...
seq_blur = iaa.Sequential([ iaa.AverageBlur(k=(2,7)), iaa.MedianBlur(k = (3,11)) ])
seq_gaussian_noise = iaa.Sequential([iaa.AdditiveGaussianNoise(scale=(20, 30))])  # add gaussian noise
seq_hue = iaa.Sequential([iaa.AddToHueAndSaturation((30, 60))])   # change their color
#seq_elastic_trans = iaa.Sequential([iaa.ElasticTransformation(alpha=90, sigma=10)])  # water-like effect
#seq_coarse_drop = iaa.Sequential([iaa.CoarseDropout((0.05, 0.1), size_percent=0.01)]) # set large image areas to zero
#seq_LC_s = iaa.Sequential([iaa.LinearContrast((1.2, 1.5))])
seq_LC_w = iaa.Sequential([iaa.LinearContrast((0.25, 0.75))])
seq_mply_l = iaa.Sequential([iaa.Multiply((0.5), per_channel=0.2)])
seq_mply_h = iaa.Sequential([iaa.Multiply((1.5), per_channel=0.2)])
seq_gray = iaa.Sequential([iaa.Grayscale(alpha=(0.8, 1.0))])

augment_dict = {'blur':seq_blur,'noise':seq_gaussian_noise,'hue':seq_hue,
        'cntr':seq_LC_w, 'mply_l':seq_mply_l, 'mply_h':seq_mply_h, 'grayscale':seq_gray}
...
def do_augment(im, aug):
    #im =  cv.cvtColor(im, cv2.COLOR_BGR2RGB)
    aug_det = augment_dict[aug].to_deterministic()
...
def get_image_blob(img):
    ...
    augment_list = ['blur','noise','hue','cntr', 'mply_l', 'mply_h', 'grayscale']
    for aug in augment_list:
        if img[aug]:
            im = do_augment(im, aug)
